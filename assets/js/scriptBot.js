...
function initializeChatBot() {
  ...
  if (!typingForm || !chatContainer || !toggleThemeButton || !deleteChatButton) {
...
document.addEventListener('components-loaded', function() {
  ...
  function initBotWithRetry(attempts = 0) {
      if (initializeChatBot()) {
          ...
          console.log(`Reintentando inicialización del bot (intento ${attempts + 1})`);
          setTimeout(() => initBotWithRetry(attempts + 1), 300 * (attempts + 1));
      } else {
          console.error('No se pudo inicializar el chat bot después de varios intentos');
      }
  }
...
setTimeout(() => {
  if (!document.querySelector('.typing-form')) {
      console.warn('El evento components-loaded no se disparó, intentando inicializar de todos modos');
      initializeChatBot();
  }
}, 3000);